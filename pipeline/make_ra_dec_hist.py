...
for file_name in file_name_list[:20]:
    dat = CDF(file_name)
...
bad_indices = np.where((ra < 0) | (ra > 360) | np.isnan(ra) | (dec < -90) | (dec > 90) | np.isnan(dec))
...
hb = ax.hexbin(ra, dec, gridsize=[360, 100], cmap="inferno", bins="log", mincnt=10)
